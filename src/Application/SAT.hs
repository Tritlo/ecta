...
getLitPathsAlg =
    CNFAlg
        { runCNF = \_ lps -> mconcat $ imap (\i lp -> LitPaths $ HashMap.map (map (ConsPath i)) $ unLitPaths lp) lps
  ...
  where
    ...
    assnNode =
        Node
            [ mkEdge
                "assignment"
                (concatMap (const [posVarNode, negVarNode]) sortedVars)
                ( mkEqConstraints $
                    map
                        (\i -> [path [2 * i, 1], path [2 * i + 1, 1]])
                        [0 .. numVars - 1]
                )
            ]
    ...
    litCopyingConstraints =
        mkEqConstraints
            [ path [0, i] : [path [1, c, i] | c <- [0 .. numClauses - 1]]
            | i <- [0 .. 2 * numVars - 1]
            ]
...
allSolutions formula = foldMap (HashSet.singleton . termToAssignment) $ getAllTerms $ fixUnbounded reducePartially $ toEcta formula
  where
    ...
    termToAssignment (Term _ [Term _ litVals, _]) =
        foldMap
            (\(var, Term "" [val, _]) -> HashMap.singleton var (termToBool val))
...
ex1 =
    And
        [ Or [PosLit "x1", PosLit "x2", PosLit "x3"]
