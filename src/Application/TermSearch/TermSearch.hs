...
generalize n@(Node [_]) =
  ...
  where
    ...
    nodeVars root =
        crush
            ( onNormalNodes $ \(Node es) ->
                filter isVar (map (\e -> Node [e]) es)
            )
            root
...
f10 =
    constFunc
        ...
        ( generalize $
            arrowType
                (arrowType var1 (arrowType var2 var2))
...
f19 =
    constFunc
        ...
        ( generalize $
            arrowType
                (arrowType var2 (arrowType var1 var2))
                (arrowType var2 (arrowType (listType var1) var2))
        )
